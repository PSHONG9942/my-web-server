...
const TILE_DISTRIBUTION = {
    'A': { count: 9, score: 1 }, 'B': { count: 2, score: 3 }, 'C': { count: 2, score: 3 }, 'D': { count: 4, score: 2 },
    'E': { count: 12, score: 1 }, 'F': { count: 2, score: 4 }, 'G': { count: 3, score: 2 }, 'H': { count: 2, score: 4 },
    'I': { count: 9, score: 1 }, 'J': { count: 1, score: 8 }, 'K': { count: 1, score: 5 }, 'L': { count: 4, score: 1 },
    'M': { count: 2, score: 3 }, 'N': { count: 6, score: 1 }, 'O': { count: 8, score: 1 }, 'P': { count: 2, score: 3 },
    'Q': { count: 1, score: 10 }, 'R': { count: 6, score: 1 }, 'S': { count: 4, score: 1 }, 'T': { count: 6, score: 1 },
    'U': { count: 4, score: 1 }, 'V': { count: 2, score: 4 }, 'W': { count: 2, score: 4 }, 'X': { count: 1, score: 8 },
    'Y': { count: 2, score: 4 }, 'Z': { count: 1, score: 10 }, '?': { count: 2, score: 0 } // Blank tiles
};
...
class Game {
    ...
    async loadDictionary() {
        try {
            const response = await fetch(`dict_${this.dictType}.json`);
            ...
            console.error(error);
            document.getElementById('loading-overlay').innerHTML = `<h2>Error loading dictionary!</h2><p>Please check your internet connection and refresh.</p>`;
        }
    }
    ...
    init() {
        ...
        this.updateUI();
        this.showMessage(`👉 ${this.players[0].name}'s Turn`);
        this.showTutorial();
    }
    ...
    restoreState(state) {
        ...
        this.updateUI();
        this.showMessage(`🔄 Reconnected! 👉 ${this.players[this.currentPlayerIndex].name}'s Turn`);
    }
    ...
    handleBoardClick(r, c) {
        ...
        if (this.isTemp(r, c)) {
            ...
            this.renderRack();
            this.showMessage("Tile recalled.");
            return;
        }
    ...
    placeTileTemp(tile, r, c) {
        if (tile.originalChar === '?') {
            let letter = prompt("Enter letter for blank tile (A-Z):");
            if (!letter) return false;
            letter = letter.toUpperCase();
            if (!/^[A-Z]$/.test(letter)) {
                alert("Invalid letter. Please enter A-Z.");
                return false;
            }
            tile.char = letter;
            tile.updateElementVisuals(tile.element);
    ...
    toggleSwapMode() {
        if (this.tempPlacedTiles.length > 0) {
            this.showMessage("❌ Recall tiles from board first!", true);
            return;
        }
        if (!this.isSwapping) {
            this.isSwapping = true;
            this.selectedTileIds.clear();
            document.getElementById('btn-swap').innerText = "✅ Confirm Swap";
            document.getElementById('btn-swap').classList.add('active');
            document.getElementById('rack-hint').innerText = "(Select tiles to discard)";
            this.showMessage("Select tiles to swap, then click button again.");
    ...
    executeSwap() {
        if (this.selectedTileIds.size === 0) {
            this.exitSwapMode();
            this.showMessage("No tiles selected, swap cancelled.");
            return;
        }
        if (this.bag.length === 0) {
            this.exitSwapMode();
            this.showMessage("❌ Bag is empty, cannot swap", true);
            return;
        }
        const player = this.players[this.currentPlayerIndex];
        const newRack = [];
        ...
        if (tilesToSwap.length > this.bag.length) {
             this.showMessage(`❌ Not enough tiles in bag (${this.bag.length} left)`, true);
             return;
        }
        ...
        this.exitSwapMode();
        this.showMessage(`♻️ Swapped ${count} tiles`);
        player.misses++;
        this.consecutivePasses = 0; 
        if (this.checkElimination(player)) return;
        this.nextTurn(true);
    ...
    checkElimination(player) {
        if (player.misses >= 3) {
            player.active = false; 
            this.showMessage(`🚫 ${player.name} eliminated (3 consecutive passes/swaps)!`, true);
    ...
    exitSwapMode() {
        this.isSwapping = false;
        this.selectedTileIds.clear();
        document.getElementById('btn-swap').innerText = "♻️ Swap";
        document.getElementById('btn-swap').classList.remove('active');
        document.getElementById('rack-hint').innerText = "(Click to select, click board to place)";
    ...
    playTurn() {
        ...
        if (this.tempPlacedTiles.length === 0) {
            this.showMessage("⚠️ Place at least one tile", true);
            return false;
        }
        ...
        if (!isRow && !isCol) {
            this.showMessage("⚠️ Tiles must be in a single row or column", true);
            return false;
        }
        ...
        if (this.isFirstTurn) {
            ...
            if (!coversCenter) { this.showMessage("⚠️ First word must cover the center star", true); return false; }
            if (lineTiles.length < 2) { this.showMessage("⚠️ Need at least 2 tiles for the first word", true); return false; }
        } else {
            ...
            if (!isConnected && !adjacentContact) { this.showMessage("⚠️ Word must connect to existing tiles", true); return false; }
        }
        ...
        if (lineTiles.length > 1) {
            ...
            if (!this.dictionary.has(word)) {
                this.showMessage(`❌ Invalid word: ${word}`, true);
                return false;
            }
            ...
            if(!formedAny) {
                 this.showMessage("⚠️ Word must connect to existing tiles", true); return false;
            }
        }
    ...
    confirmMove(points, wordsFormed, isBingo) {
        ...
        let msg = `🎉 Score: ${points} (${wordsFormed.join(', ')})`;
        if(isBingo) msg = `🔥 BINGO! +50 pts! ` + msg;
    ...
    nextTurn(isValidMove = false) {
        ...
        this.updateUI();
        let msg = `👉 ${this.players[this.currentPlayerIndex].name}'s Turn`;
    ...
    endGame(isForced = false, finishingPlayer = null) {
        ...
        if (isForced) {
            msgHTML += `<div style="color:#e76f51; margin-bottom:10px;">🚫 Match ended due to consecutive passes.</div>`;
        } else if (finishingPlayer) {
            msgHTML += `<div style="color:#2a9d8f; margin-bottom:10px;">🎉 ${finishingPlayer.name} finished all tiles!</div>`;
        }
        ...
        if (winner.score < 0) {
            msgHTML += `<div style="color:#ffca3a; margin-bottom:10px; font-style: italic;">
                "Scrabble is a game of skill and vocabulary, not just a test of who can use the most hints! Better luck next time!"
            </div>`;
        }
        
        let tableHTML = `<table class="endgame-table">
            <tr><th>Rank</th><th>Player</th><th>Status</th><th>Final Score</th></tr>`;
        this.players.forEach((p, index) => {
            const status = p.active ? 'Finished' : 'Eliminated';
        ...
        tableHTML += `</table>`;
        document.getElementById('modal-title').innerText = "🏆 Final Results";
